...
export default function Contact() {
  ...
  useEffect(() => {
    // Image reveal: slow from top
    gsap.fromTo(
      imageWrapperRef.current,
      { height: 0, opacity: 0 },
      { height: "100%", opacity: 1, duration: 3, ease: "power2.out" }
    );

    // Form reveal: slide from left with fade in
    gsap.fromTo(
      formRef.current,
      { x: -100, opacity: 0 },
      { x: 0, opacity: 1, duration: 2, ease: "power2.out", delay: 0.5 }
    );
  }, []);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-full">
      ...
      <div
        className="order-1 lg:order-2 w-full lg:w-1/2 h-[510px] lg:h-full relative overflow-hidden"
      >
      ...
      <div
        ref={formRef}
        className="order-2 lg:mx-10 lg:order-1 flex justify-between items-center w-full lg:w-1/2 p-10 opacity-0"
      >
